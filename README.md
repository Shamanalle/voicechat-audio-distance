...
1. Put `voicechat-audio-distance-bukkit-1.2.2.jar` into the server's `plugins/` folder, next to the Bukkit version of Simple Voice Chat.
...
1. Положите `voicechat-audio-distance-bukkit-1.2.2.jar` в папку `plugins/` сервера, рядом с версией Simple Voice Chat для Bukkit.
